...
- 変数名は長さではなく、スコープ内で表す実態が読み取れるかを基準にする。
- `err`、`ok` など、Go の慣習や狭いスコープで実態に合う短い変数名は許容する。
- index を表す変数は `i` ではなく `index` と書く。複数の index や比較対象がある場合も、役割が伝わる名前を使う。
- 意味が曖昧になる場合は短縮せず、業務語彙や役割が伝わる名前を使う。
- 関数とメソッドの戻り値は、原則として名前付き戻り値にする。
- 戻り値名は呼び出し側の意味ではなく、関数内で扱う実態に合わせる。
- `error` は `err`、結果の有無を表す `bool` は `ok` など、Go の慣習に合う短い戻り値名は許容する。
- 関数とメソッドの引数と戻り値は、1項目ずつ改行して表示する。
